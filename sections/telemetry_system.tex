...
\subsection{Uorb \cite{uorb-driver}}

Uorb (micro object request broker) is a pub-sub message broker that NuttX offers as a driver interface, it's advantages over the traditional POSIX \texttt{open()} and \texttt{close()} interface mentioned above
include callback based notifications instead of polling, internal circular buffers for multi client reads and an additional abstaction layer that separates sensors by type.

To implement the uorb interface a driver must register as a node of topic type (temperature, mass, gps), this creates a character device that when subscribed to triggers a user specified callback with the sensor values.
We have been transitioning all of our drivers to uorb as it provides us with increased data collection capabilities at a smaller footprint.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{assets/diagrams/uorb.png}
    \caption{Uorb wrapper model}
    \label{fig:uorb}
\end{figure}

...
into a single combined design to save space in the rocket and money on board manufacturing costs. The radio board was
kept seperate because of the frequency with which it is redesigned and so it could be replaced with a \gls{cots} radio
board if the design \gls{cuinspace} produced did not have the required range, however it could also be incorporated in
future years.
...
\start{itemize}
    \item Removal of a version field for packet headers, as the team has not supported older versions of the

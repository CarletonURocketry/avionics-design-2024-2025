...
communication over a network. The primary components of the network control system are Ubiquity Litebeam M5 network bridges for
long range wireless communication, pictured in Figure \ref{fig:ubiquity-dish}. This avoids the need for Ethernet cabling
...
The telemetry clients and control clients will each be connected via ethernet to a network switch in addition to a Ubiquity Litebeam
M5 network bridge. These components will make up the control base \gls{lan}

The pad server node will be placed at the hybrid launch pad site. It will be connected directly via an ethernet cable to a Ubiquity
Litebeam M5 network bridge. The pad server node and the network bridge will make up the launch pad \gls{lan}

The two LANs will be connected wirelessly by the network bridges configured in bridge mode. 
